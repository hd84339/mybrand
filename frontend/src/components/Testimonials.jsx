...
const Testimonials = () => {
    ...
    return (
        <section ref={sectionRef} className="py-24 bg-white relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-[40rem] h-[40rem] bg-indigo-50/50 rounded-full blur-3xl -z-10 pointer-events-none"></div>

            <div className="container mx-auto px-6">

                {/* Header */}
                <div ref={headerRef} className="text-center max-w-3xl mx-auto mb-20 opacity-0">
                    <span className="inline-block py-1 px-3 mb-4 rounded-full bg-orange-50 border border-orange-100 text-xs font-bold text-orange-600 tracking-wider uppercase">
                        Success Stories
                    </span>
                    <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mb-6">
                        Trusted by <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-fuchsia-600">Ambitious Brands</span>
                    </h2>
                ...
                <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {testimonials.map((testimonial, index) => (
                        <div key={index} className="group bg-slate-50/50 backdrop-blur-sm p-8 rounded-2xl border border-slate-100 hover:shadow-xl hover:shadow-indigo-100/50 hover:-translate-y-2 transition-all duration-300 opacity-0 flex flex-col">
